...
<div> 
	<div id="contenuto">
		...
		<div  id="formRicerca">
				...
				<tr>
				 ...
				 <tr>
					...
					<select name="emanante"  style="width:411px;">
						<option value=""></option>
						<option value="LAVORO">Ministero del Lavoro e delle Politiche Sociali</option>
						<option value="FINANZE">Ministero dell'Economia e delle Finanze</option>
						<option value="INTERNO">Ministero dell'interno </option>
						<option value="GIUSTIZIA">Ministero della giustizia </option>
						<option value="SANITA">Ministero della sanità</option>
						<option value="INNOVAZIONE">Ministero per l'innovazione e le tecnologie</option>
						<option value="INNOVAZIONE">Pres Cons Ministri - dip. per l'innovazione e le tecnologie </option>
						<option value="AMMINISTRAZIONE">Pres Cons Ministri - dip. per le riforme e le innovazioni nella PA</option>
							{ <div/> 
							(:
								let $colName := "/db/nir/normeCnipa"
			...
			<table witdh="500px">
					<tr>
							<th align="center" width="280px">Materia</th><th align="center" width="220px">Pubblicazione</th>
						</tr>
						<tr>
							<td  align="right" >voce 
								<select name="materia"  style="width:187px;">
								{
									for $valore in distinct-values(collection($colName)//iit:materia/@valore)
										order by $valore 
										return <option>{if ($valore eq $materia) then attribute selected {"selected"} else ()}{$valore cast as xs:string}</option>
								}
								</select></td>
								<td  align="right" >Fonte 
								<select name="fonte" style="width:50px;">
									<option/>
								{
									for $tipoFonte in distinct-values(collection($colName)//nir:pubblicazione/@tipo)
										order by $tipoFonte 
										return <option>{if ($tipoFonte eq $fonte) then attribute selected {"selected"} else ()}{$tipoFonte cast as xs:string}</option>
								}
								</select>
					...
					<tr>
							...
							<td  align="right"  colspan="2">Tipo 
								  <select name="tipoModifica"   class="low" >
									    <option></option>
									    <option>decreto.legge</option>
									    <option>decreto.legislativo</option>
									    <option>legge</option>
									    <option>decreto.presidente.consiglio.ministri</option>
									</select></td>
						...
						<tr> <td colspan="3"  align="center" style="FONT-STYLE:italic;">N.B. vanno compilati tutti i campi</td></tr>
						</table><br/>
			
			<table width="500px">
			<tr>	<th align="center" colspan="3">Contiene riferimenti</th>
						</tr>
						<tr>
							<td  align="right"  colspan="2">Tipo 
...
declare function search:displayRif($res as node(),$urn as xs:string,$url as xs:string) as element()*{
	for $ref in $res//nir:rif[contains(@xlink:href, $urn)]
		return 
		if (empty($ref/ancestor::nir:comma/@id)) then () else
		<li><a target="_new" href="./ui/ui.html?doc={$url}#{$ref/ancestor::nir:comma/@id}">{$ref/ancestor::nir:comma/@id cast as xs:string}</a></li>
...
declare function search:displayComma($res as node(),$testo as xs:string,$url as xs:string) as element()*{
	for $comma in $res//nir:comma 
		where $comma &= $testo
		return <li><a target="_new" href="./ui/ui.html?doc={$url}#{$comma/@id}">{if (empty($comma/@id)) then () else $comma/@id cast as xs:string}</a></li>
...
declare function search:displayArticolo($res as node(),$testo as xs:string,$url as xs:string) as element()*{
	for $articolo in $res//nir:articolo
		...
		return <li><a target="_new" href="./ui/ui.html?doc={$url}#{$articolo/@id}">{if (empty($articolo/@id)) then () else $articolo/@id cast as xs:string}</a></li>
...
declare function search:makeResName($res as node(),$url as xs:string) as element()*{
	...
	let $titoloDoc := if ($res/nir:NIR/*/nir:intestazione/nir:titoloDoc[1]/text()) then concat(" - ",$res/nir:NIR/*/nir:intestazione/nir:titoloDoc[1] cast as xs:string) else ()
		return <span>
					<a target='_new' href='./ui/ui.html?doc={$url}&amp;datafine={date:normalize-date(current-dateTime())}'>{concat($tipoDoc, $emanante, $dataDoc, $numDoc,$titoloDoc)}</a>
...
declare function search:search() as element()* {
	...
	let $query8 := if ($titolo eq "") then "" else concat(" and $res/nir:NIR/*/nir:intestazione/nir:titoloDoc ", $opApertura," $titolo")
	let $query9 := if ($combinazione ne "comma" or $testo eq "") then "" else " and $res//nir:comma[. &amp;= $testo]"
	
	let $urnRif := concat("urn:nir:stato:",$tipoRiferimento,":",$annoRiferimento,"-",$meseRiferimento,"-",$giornoRiferimento,";",$numeroRiferimento)  
	let $query10 := if ($urnRif eq "urn:nir:stato::--;") then "" else " and $res//nir:rif/contains(@xlink:href, $urnRif )" 
	
	let $urnRelazione := concat("urn:nir:stato:",$tipoModifica,":",$annoModifica,"-",$meseModifica,"-",$giornoModifica,";",$numeroModifica)
	let $query11 :=  if ($urnRelazione eq "urn:nir:stato::--;" ) then "" else " and $res//nir:ciclodivita/nir:relazioni/nir:passiva/contains(@xlink:href, $urnRelazione)"
	...
	let $query13 := "
				...
				<li > 
					...
					{if ($urnRif eq 'urn:nir:stato::--;') then () else
						<ul>
