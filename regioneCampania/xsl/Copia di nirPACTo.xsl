...
<!--	<xsl:template match="nir:comma | nir:el | nir:en | nir:ep">
			...
			<xsl:when test="contains($url,'urn:nir:')">
				<xsl:choose>
					<xsl:when test="$strina='&#59;' or $strina='&#46;' or $strina='&#58;' or $strina='&#44;'  or $strina='&#45;'">		 
						<a href="http://www.pacto.it/nirres/nirres.php?URN={@xlink:href}" title="{@xlink:href}">
							<xsl:apply-templates/>
						</a>
					</xsl:when>
					<xsl:otherwise>					 
		...
		<xsl:for-each select="//@iniziovigore">
					...
					<xsl:variable name="fonte">
						<xsl:choose>
							<xsl:when test="$fine_id!=''">
								<xsl:value-of select="//*[name()='evento'][@fonte=$fine_id]/@fonte"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="//*[name()='evento'][@fonte=$inizio_id]/@fonte"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
			...
			<xsl:choose>
				<xsl:when test="/*[name()='NIR']/*/*[name()='meta']/*[name()='disposizioni']/*[name()='modifichepassive']/*/*/*[name()='dsp:pos'][@xlink:href=$id]/../../*[name()='dsp:norma']">
					...
					<a href="http://www.pacto.it/nirres/nirres.php?URN={@xlink:href}" title="{@xlink:href}"><xsl:value-of select="$autonota"/></a>
					<xsl:text>. </xsl:text>
			      </xsl:for-each>
				</xsl:when>
				...
				<xsl:otherwise>
					...
					<a href="http://www.pacto.it/nirres/nirres.php?URN={@xlink:href}" title="{@xlink:href}"> <xsl:value-of select="$urn" /> </a>
					<xsl:text>. </xsl:text>
				</xsl:otherwise>			
			</xsl:choose>
				<xsl:choose>
   ...
   <xsl:template name="TimeLine">
   	...
   	<p>Vedi documento vigente alla data:</p>
   	<ul id="choosedata">
   		<xsl:text></xsl:text>
		...
		<xsl:variable name="pos">
			...
			<xsl:otherwise>
		 		<li class="choosedata">
					<a
						href="?doc={$doc}&amp;xml={$xml}&amp;datafine={@data}" title="vedi documento vigente a {$data}">
						<xsl:value-of select="$data" />
						<xsl:if test="$pos=1">
							<xsl:text> (Originale) </xsl:text>
						</xsl:if>
   	...
   	<p>Vedi documento multivigente:</p>
   	<ul>
		<xsl:choose>
			<xsl:when test="$datafine=''">
		 		<li class="choosedata-active">
			...
			<xsl:otherwise>
		 		<li class="choosedata">
					<a
						href="?xml={$xml}&amp;datafine"  title="vedi documento multivigente">
						Multivigente
					</a>
				</li>
			</xsl:otherwise>
