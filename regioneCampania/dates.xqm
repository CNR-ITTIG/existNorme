...
declare function date:normalize-date($date as xs:dateTime) as xs:string {
	concat(xs:string(year-from-date($date)),date:format-int(month-from-date($date)),date:format-int(day-from-date($date)))
};
