...
let $tmp := local:createCollection("//db/nir", "normeCnipa")
let $tmp := xmldb:chmod-collection("/db/nir/normeCnipa", 508)

let $modulePath := system:get-module-load-path()

let $tmp:= xmldb:store-files-from-pattern("/db/nir/normeCnipa", concat($modulePath, "/repository/normeCnipa"), "**/*.xml", "text/xml", true()) 



	return  <init modulePath="{$modulePath}">{$tmp}</init>
