...
<script type="text/javascript" src="js/styleswitcher.js">&#160;</script>
    ...
    <div id="page_head_navi">
	    ...
	    <a href="?doc={$doc}&amp;datafine={$datafine}" title="Stile Normale" ><img src="img/color.gif" alt="[Style default]" /></a>
	    <a href="?doc={$doc}&amp;datafine={$datafine}&amp;css=2" title="Stile Alto contrasto - Bianco/Nero" ><img src="img/altocon.gif" alt="[Bianco/Nero]" /></a>
	    <a href="?doc={$doc}&amp;datafine={$datafine}&amp;css=3" title="Stile solo testo" ><img src="img/solotesto.gif" alt="[Solo Testo]" /></a>
    </div>
...
<div id="menuHor">
	...
	<xsl:template match="//*[name()='numDoc']">
		<span> 
			<xsl:choose>
				<xsl:when test="@nome='protocollo'">
					<xsl:text> (protocollo n. </xsl:text><xsl:value-of select="."/><xsl:text>)</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="."/>
				</xsl:otherwise>							
	...
	<xsl:template match="//*[name()='rifesterno']">
		<a href="urnResolver.xql?css={$css}&amp;urn={@xlink:href}" title="URN = {@xlink:href}" ><xsl:value-of select="@xlink:href" /></a>
	</xsl:template>
...
-->
					...
					<a href="urnResolver.xql?css={$css}&amp;urn={$urn_meta}" title="URN = {$urn_meta}"><xsl:value-of select="$autonota"/></a>
					<xsl:text>. </xsl:text>
			      </xsl:for-each>
				</xsl:when>
				<xsl:otherwise>
   				...
   				<li>
						...
						<xsl:when test="$pos=1">
							...
							<xsl:choose>
								...
								<xsl:otherwise><a href="?css={$css}&amp;doc={$doc}"  title="vedi documento multivigente"> multivigente</a></xsl:otherwise>
							</xsl:choose>
							</div>
