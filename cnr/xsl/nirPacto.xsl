			...
			<xsl:when test="contains($url,'urn:nir:')">
				<xsl:choose>
					<xsl:when test="$strina='&#59;' or $strina='&#46;' or $strina='&#58;' or $strina='&#44;'  or $strina='&#45;'">		 
						<a href="urnResolver.xql?urn={@xlink:href}" title="{@xlink:href}">
							<xsl:apply-templates/>
						</a>
					</xsl:when>
					<xsl:otherwise>					 
		...
		<xsl:for-each select="//@iniziovigore">
			...
			<xsl:choose>
				<xsl:when test="/*[name()='NIR']/*/*[name()='meta']/*[name()='disposizioni']/*[name()='modifichepassive']/*/*/*[name()='dsp:pos'][@xlink:href=$id]/../../*[name()='dsp:norma']">
					...
					<a href="urnResolver.xql?urn={@xlink:href}" title="{@xlink:href}"><xsl:value-of select="$autonota"/></a>
					<xsl:text>. </xsl:text>
			      </xsl:for-each>
				</xsl:when>
				...
				<xsl:otherwise>
					...
					<a href="urnResolver.xql?urn={@xlink:href}" title="{@xlink:href}"> <xsl:value-of select="$urn" /> </a>
					<xsl:text>. </xsl:text>
				</xsl:otherwise>			
			</xsl:choose>
				<xsl:choose>
   	...
   	<ul>
		<xsl:choose>
			...
			<xsl:otherwise>
		 		<li class="choosedata">
					<a
						href="?doc={$doc}&amp;xml={$xml}"  title="vedi documento multivigente">
						Multivigente
					</a>
				</li>
			</xsl:otherwise>
