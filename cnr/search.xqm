...
<div> 
	<div id="contenuto">
		...
		<H3>Raccolta normativa Consiglio Nazionale delle Ricerche</H3>
...
-->	
                                            ...
                                            <!--td align="right" width="190">Tipo 
												<select id="estremiTipo" name="tipo" style="width:143;height:22">
                                                    ...
                                                    <option value="provvedimento">{if ($tipo eq 'provvedimento') then attribute selected {"selected"} else ()}Provvedimento CNR</option>
                                                    <option value="decreto legislativo">{if ($tipo eq 'decreto legislativo') then attribute selected {"selected"} else ()}Decreto legislativo</option>
                                                </select>
											</td-->
											
											
						<td align="right" width="190">Tipo <select id="estremiTipo" name="tipo" style="width:143;height:22">
                                                    <option/>
											{
												for $valore in distinct-values(collection($colName)//iit:tipoDoc/@valore)
...
declare function search:makeResName($res as node(),$url as xs:string) as element()*{
	...
	let $titoloDoc := if ($res/nir:NIR/*/nir:intestazione/nir:titoloDoc[1]/text()) then concat("",$res/nir:NIR/*/nir:intestazione/nir:titoloDoc[1] cast as xs:string) else ()
		return <span>
					<a target='_new' href='xhtml?doc={$url}&amp;datafine={date:normalize-date(current-dateTime())}'>{concat($tipoDoc,$emanante,$dataDoc,$numDoc,$numProtocollo) }</a>
					<br/>{$titoloDoc}
