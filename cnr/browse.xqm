...
{
    let $colName := request:get-parameter("collection", "/db/cnr/norme") return
        <div id="browsePanel">
			<div>
...
declare function browse:upload($colName as xs:string) as element()
{
	...
	let $sottotipo := doc($tmp)/nir:NIR/nir:DocumentoNIR/@nome cast as xs:string
	let $tipo := local:tipoDoc($sottotipo , name(doc($tmp)/nir:NIR/*))
	...
	let $tmp := local:createCollection("/db/cnr/history", $docName)
	let $verName := if (doc($PathDoc)//iit:version/@valore) then doc($PathDoc)//iit:version/@valore cast as xs:string  else ()
	let $tmp:= if (doc($PathDoc)) then xdb:copy($colName, concat("/db/cnr/history/", $docName), $docName) else () 
	let $tmp:= if (doc($PathDoc)) then xdb:rename(concat("/db/cnr/history/", $docName), $docName, $verName) else () 
            ...
            <ul>
		                <li>Storing uploaded content to: {$PathDoc}</li>
			                { (: xdb:decode-uri(xs:anyURI(xdb:store($colName, xdb:encode-uri($urn), $file))), :)
			                    if (exists(doc($PathDoc)/nir:NIR/*/nir:meta/nir:proprietario[@soggetto="IIT"] )) then 
			                  update replace doc($PathDoc)/nir:NIR/*/nir:meta/nir:proprietario[@soggetto="IIT"] with $meta
									else
